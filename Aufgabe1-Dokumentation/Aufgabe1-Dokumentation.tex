...
\end{wrapfigure}
 ...
 \begin{figure}[H]
 	\centering \includesvg[width=0.25\textwidth]{pics/lisarennt1.svg}
...
Insgesamt ergibt sich für die Laufzeit aller drei Algorithmen also $\mathcal{O}(n^2 \cdot \log n + n^2 \cdot \log n + n) = \mathcal{O}(n^2 \cdot \log n)$.
 ...
 %Die Erweiterung läuft pro Punkt in konstanter Laufzeit, also insgesamt in $\mathcal{O}(n)$, da jeder Punkt transformiert werden muss. Anschließend muss eine Rücktransformation erfolgen Diese Erweiterung wurde \textbf{implementiert}.
...
\begin{algorithmic}
	...
	\State $v_{k+1} = v_1$; $v_{k+2} = v_2$; $w_{m+1} = w_1$; $w_{m+2} = w_2$
...
\section{Umsetzung}
%TODO Eingabegeschwindigkeiten km/h -> m/s
...
%TODO Quellcode Minkowski und Pyvisgraph
